...
class Lightsail {
  ...
  async getGroupBlueprintId(group) {
    await this.init();
    let version =
      this.args && this.args.blueprintVersion
        ? this.args.blueprintVersion
        : '*';
    const groupBlueprints = await this.getGroupBlueprints(group);
    // Return exact match, if exists.
    if (groupBlueprints[version]) {
      return groupBlueprints[version];
    }
    // Try matching by semver.
    const versions = Object.keys(groupBlueprints).sort(semver.rcompare);
    version = semver.validRange(version);
    if (version == null) {
      throw new AWSUtilsError('Invalid semver version range.');
    }
  ...
  async createInstance(group) {
    ...
    for (let i = 0; i < zones; i++) {
      ...
      if (this.args.userData) {
        params.userData = this.args.userData;
      }
      await this._ls.createInstances(params);
    }
    return result;
  }
